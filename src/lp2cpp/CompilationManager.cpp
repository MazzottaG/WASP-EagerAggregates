...
void CompilationManager::generateStratifiedCompilableProgram(aspc::Program & program, AspCore2ProgramBuilder* builder) {
    ...
    *out << ind << "\n";
    *out << ind << "const std::vector<int> EMPTY_TUPLES;\n";
                    ...
                    if(aggrId->getAriety() == 0){
                        ...
                        *out << ind << "possibleSum[itAggrId]-=tuple->at("<<sumVar<<");\n";
                        ...
                        for(std::string sign : {"p","u","f"}){
                            *out << ind++ << "{\n";
                                *out << ind << "const std::vector<int>* aggrIds = &"<<sign<<mapName<<".getValues({"<<terms<<"});\n";
        ...
        for(int component=scc.size()-1; component>=0 ; component--){
            for(unsigned predId : scc[component]){
                ...
                if(it!=vertexByID.end()){
                        ...
                        if(builder->isAuxPredicate(it->second.name)!=0)
                            std::cout<<"    Is Aux Pred"<<std::endl;
                        else if(builder->isAuxValPred(it->second.name)!=0)
                            std::cout<<"    Is Aux Pred"<<std::endl;
                        else if(builder->isBodyPredicate(it->second.name)!=0)
                            std::cout<<"    Is Body Pred"<<std::endl;
                        else if(builder->isAggrSetPredicate(it->second.name)!=0)
                            std::cout<<"    Is AggrSet Pred"<<std::endl;
                    #endif
                            ...
                            if(selectedIneq != NULL){
                                ...
                                if(selectedLit!=NULL){
                                    ...
                                    if(selectedLit->isBoundedLiteral(boundVariables)){
                                        ...
                                        *out << ind << "const std::vector<int>* tuples = &p"<<selectedLit->getPredicateName()<<"_";
                                        ...
                                        *out << ind << "const std::vector<int>* tuplesU = &u"<<selectedLit->getPredicateName()<<"_";
                        ...
                        *out << --ind << "}\n";
                        ...
                        for(const aspc::Rule& r : program.getRules()){
                            ...
                            if(!r.isConstraint() && r.getHead()[0].getPredicateName()==it->second.name && !r.containsAggregate()){
                                #ifdef TRACE_PROP_GEN
                                    std::cout<<"    Rule found for predicate without aggregate"<<std::endl<<"       ";
                                    r.print();
                                #endif
                                if(r.getBodyLiterals().size()==1 && auxToBody.count(r.getBodyLiterals()[0].getPredicateName())==0){
                                    ...
                                    *out << ind++ << "{\n";
                                        *out << ind << "const std::vector<int>* tuples = &p"<<bodyLit->getPredicateName()<<"_.getValues({});\n";
                                        *out << ind << "const std::vector<int>* tuplesU = &u"<<bodyLit->getPredicateName()<<"_.getValues({});\n";
                                        *out << ind++ << "for(unsigned i=0; i<tuples->size()+tuplesU->size(); i++){\n";
                                            *out << ind << "const Tuple* tuple = NULL;\n";
                                            *out << ind++ << "if(i<tuples->size()){\n";
                                                *out << ind << "tuple=factory.getTupleFromInternalID(tuples->at(i));\n";
        ...
        for(const auto& aggrSetPred : aggrSetToRule){
            for(unsigned ruleId : aggrSetPred.second){
                ...
                if(aggregateRelation->getAggregate().isSum()){
                    ...
                    *out << ind++ << "{\n";
                        *out << ind << "const std::vector<int>* aggregateIds = &u"<<aggrId->getPredicateName()<<"_.getValues({});\n";
                        *out << ind++ << "for(unsigned i=0;i<aggregateIds->size();i++){\n";
                            *out << ind << "int it = aggregateIds->at(i);\n";
                            *out << ind << "const Tuple* currentTuple = factory.getTupleFromInternalID(aggregateIds->at(i));\n";
                            ...
                            *out << ind++ << "for(unsigned j=0; j<aggrSetTuples->size(); j++)\n";
                                *out << ind-- << "possibleSum[it]+=factory.getTupleFromInternalID(aggrSetTuples->at(j))->at("<<sumVar<<");\n";
    ...
    *out << --ind << "}\n";
    ...
    // ---------------------- end init() --------------------------------------//
    *out << ind++ << "bool propUndefined(const Tuple* tupleU,bool isNegated,std::vector<int>& stack,bool asNegative,std::vector<int> & propagatedLiterals,std::unordered_set<int> & remainingPropagatingLiterals,const Solver* solver){\n";
                ...
                ind++;
                    ...
                    *out << ind++ << "if (insertResult.second) {\n";
                        ...
                        *out << ind << "insertTrue(insertResult);\n";
                        ...
                        // *out << ind << "std::cout<<\"Lite1ral propagated as True\";";
        ...
        ind++;
            ...
            *out << ind++ << "if(remainingPropagatingLiterals.count(it*sign)==0){\n";
                ...
                *out << ind << "propagatedLiterals.push_back(it*sign);\n";
                ...
                // *out << ind << "if(solver == NULL) std::cout<<\"Null solver\"<<std::endl;else std::cout<<\"Solver not null\"<<std::endl;\n";
                *out << ind << "Activity val = solver->getActivityForLiteral(propagatedLiterals.back());\n";
                // *out << ind << "std::cout<<\"Prop literal activity: \"<<val<<std::endl;\n";
                ...
                *out << ind++ << "if(conflictualSize < propagatedLiterals.size()){\n";
                    *out << ind++ << "for(int i=conflictualSize-1; i >= 0; i--){\n";
                        ...
                        // *out << ind << "std::cout<<\"Comparing\"<<std::endl;\n";
                        *out << ind << "Activity occurs = solver->getActivityForLiteral(propagatedLiterals[i]);\n";
                        // *out << ind << "std::cout<<\"Comp literal activity: \"<<occurs<<std::endl;\n";
...
void CompilationManager::compileEagerRuleWithAggregate(const aspc::Rule& r,bool fromStarter){
    ...
    if(fromStarter){
        {
            ...
            *out << ind << "std::cout<<\"Prop rule with aggr\"<<std::endl;\n";
            ...
            for(unsigned i = 0; i<aggrIdAtom->getAriety(); i++){
                ...
                *out << ind << "const std::vector<int>* tuples = &p"<<mapName<<".getValues(sharedVar);\n";
                *out << ind << "const std::vector<int>* tuplesU = &u"<<mapName<<".getValues(sharedVar);\n";
                string bodyTerms = "";
                if(bodyPred!=NULL){
                        ...
                        *out << ind << "std::vector<int> reason;\n";
                        if(builder->isAggrSetPredicate(aggrSetPred->getPredicateName())){
                            *out << ind++ << "for(int index=0; index<tuplesU->size();){\n";
                                *out << ind << "const Tuple* currentTuple = factory.getTupleFromInternalID(tuplesU->at(index));\n";
                                *out << ind++ << "if(actualSum[uStartVar]+currentTuple->at(0) >= "<<guard<<"){\n";
                                    ...
                                    *out << ind << "propUndefined(currentTuple,false,propagationStack,true,propagatedLiterals,remainingPropagatingLiterals, solver);\n";
                                *out << --ind << "}else index++;\n";
                            *out << --ind << "}\n";
                        }else{
                            std::string firstAggrVar = aggregateRelation->getAggregate().getAggregateVariables()[0];
                            ...
                            *out << ind++ << "for(unsigned index =0; index<tuplesU->size(); index++){\n";
                                *out << ind << "const Tuple* currentTuple = factory.getTupleFromInternalID(tuplesU->at(index));\n";
                                *out << ind++ << "if(actualSum[uStartVar]+currentTuple->at("<<varIndex<<") >= "<<guard<<"){\n";
                                    *out << ind << "int itProp = currentTuple->getId();\n";
                                        *out << ind++ << "if(reasonForLiteral.count(-itProp)==0 || reasonForLiteral[-itProp].empty()){\n";
                                        *out << ind++ << "if(reason.empty()){\n";
                                            *out << ind++ << "for(unsigned i =0; i< tuples->size(); i++){\n";
                            ...
                            *out << ind << "reason.push_back(startVar);\n";
                            if(builder->isAggrSetPredicate(aggrSetPred->getPredicateName())){
                                *out << ind++ << "while(!tuplesU->empty()){\n";
                                    ...
                                    *out << --ind << "}\n";
                                    //*out << ind << "std::cout<<\"Propagating from rule: "<<r.getRuleId()<<"\"<<std::endl;\n";
                                    *out << ind << "propUndefined(factory.getTupleFromInternalID(tuplesU->at(0)),false,propagationStack,true,propagatedLiterals,remainingPropagatingLiterals, solver);\n";
                                *out << --ind << "}\n";
                            }else{
                                *out << ind++ << "for(unsigned i =0; i<tuplesU->size(); i++){\n";
                                    ...
                                    *out << --ind << "}\n";
                                    //*out << ind << "std::cout<<\"Propagating from rule: "<<r.getRuleId()<<"\"<<std::endl;\n";
                                    *out << ind << "propUndefined(factory.getTupleFromInternalID(tuplesU->at(i)),false,propagationStack,true,propagatedLiterals,remainingPropagatingLiterals, solver);\n";
                                *out << --ind << "}\n";
                            }
                    ...
                    ind++;
                        if(builder->isAggrSetPredicate(aggrSetPred->getPredicateName())){
                            *out << ind++ << "for(unsigned index=0;index<tuplesU->size();){\n";
                                *out << ind << "const Tuple* currentTuple = factory.getTupleFromInternalID(tuplesU->at(index));\n";
                                *out << ind++ << "if(actualSum[uStartVar]+possibleSum[uStartVar]-currentTuple->at(0) < "<<guard<<"){\n";
                                    ...
                                    *out << ind << "propUndefined(currentTuple,false,propagationStack,false,propagatedLiterals,remainingPropagatingLiterals, solver);\n";
                                *out << --ind << "}else index++;\n";
                            *out << --ind << "}\n";
                        }else{
                            std::string firstAggrVar = aggregateRelation->getAggregate().getAggregateVariables()[0];
                            ...
                            *out << ind++ << "for(unsigned index=0;index<tuplesU->size();index++){\n";
                                *out << ind << "const Tuple* currentTuple = factory.getTupleFromInternalID(tuplesU->at(index));\n";
                                *out << ind++ << "if(actualSum[uStartVar]+possibleSum[uStartVar]-currentTuple->at("<<varIndex<<") < "<<guard<<"){\n";
                                    *out << ind << "int itProp = tuplesU->at(index);\n";
                                    *out << ind++ << "if(reasonForLiteral.count(itProp) == 0 || reasonForLiteral[itProp].empty()){\n";
                                        *out << ind << "const std::vector<int>* tuplesF = &f"<<mapName<<".getValues(sharedVar);\n";
                                        *out << ind++ << "for(unsigned i = 0; i < tuplesF->size(); i++){\n";
                        ...
                        if(builder->isAggrSetPredicate(aggrSetPred->getPredicateName())){
                            *out << ind++ << "while(tuplesU->size()>0){\n";
                                ...
                                *out << --ind << "}\n";
                                    //*out << ind << "std::cout<<\"Propagating from rule: "<<r.getRuleId()<<"\"<<std::endl;\n";
                                // reason contains aggr_id and all aggr_set false
                                *out << ind << "propUndefined(factory.getTupleFromInternalID(tuplesU->at(0)),false,propagationStack,false,propagatedLiterals,remainingPropagatingLiterals, solver);\n";
                            *out << --ind << "}\n";
                        }else{
                            *out << ind++ << "for(unsigned index=0;index<tuplesU->size();index++){\n";
                                *out << ind << "int itProp = tuplesU->at(index);\n";
                                ...
                                *out << --ind << "}\n";
                                    //*out << ind << "std::cout<<\"Propagating from rule: "<<r.getRuleId()<<"\"<<std::endl;\n";
                                // reason contains aggr_id and all aggr_set false
                                *out << ind << "propUndefined(factory.getTupleFromInternalID(tuplesU->at(index)),false,propagationStack,false,propagatedLiterals,remainingPropagatingLiterals, solver);\n";
                            *out << --ind << "}\n";
                        }
                        *out << --ind << "}\n";
                    }
    ...
    }
    {
        ...
        if(bodyPred!=NULL){
            ...
            *out << ind << "const std::vector<int>* tuples = &p"<<aggrIdAtom->getPredicateName()<<"_.getValues({});\n";
            *out << ind << "const std::vector<int>* tuplesU = &u"<<aggrIdAtom->getPredicateName()<<"_.getValues({});\n";
            *out << ind << "const std::vector<int>* tuplesF = &f"<<aggrIdAtom->getPredicateName()<<"_.getValues({});\n";
            ...
            *out << ind++ << "for(unsigned i = 0; i<tuples->size(); i++){\n";
            {
                ...
                *out << ind << "const std::vector<int>* joinTuples = &p"<<mapName<<".getValues(sharedVar);\n";
                *out << ind << "const std::vector<int>* joinTuplesU = &u"<<mapName<<".getValues(sharedVar);\n";
                    ...
                    *out << --ind << "}else{\n";
                    ...
                    if(builder->isAggrSetPredicate(aggrSetPred->getPredicateName())){
                        *out << ind++ << "for(unsigned index=0; index<joinTuplesU->size();){\n";
                            ...
                            *out << ind << "if(actualSum[aggrIdIt]+possibleSum[aggrIdIt]-currentJoinTuple->at(0) >= "<<guard<<") {index++; continue;}\n";
                            ...
                            *out << --ind << "}\n";
                                    //*out << ind << "std::cout<<\"Propagating from rule: "<<r.getRuleId()<<"\"<<std::endl;\n";
                            // reason contains aggr_id and all aggr_set false
                            *out << ind << "propUndefined(currentJoinTuple,false,propagationStack,false,propagatedLiterals,remainingPropagatingLiterals, solver);\n";
                        *out << --ind << "}\n";
                    }else{
                        std::string firstAggrVar = aggregateRelation->getAggregate().getAggregateVariables()[0];
                        unsigned varIndex = 0;
                        ...
                        *out << ind++ << "for(unsigned index=0; index<joinTuplesU->size(); index++){\n";
                            *out << ind << "const Tuple* currentJoinTuple = factory.getTupleFromInternalID(joinTuplesU->at(index));\n";
                            *out << ind << "if(actualSum[aggrIdIt]+possibleSum[aggrIdIt]-currentJoinTuple->at("<<varIndex<<") >= "<<guard<<") {index++; continue;}\n";
                            *out << ind << "int itProp = joinTuplesU->at(index);\n";
                            *out << ind++ << "if(reasonForLiteral.count(itProp) == 0 || reasonForLiteral[itProp].empty()){\n";
                                *out << ind << "const std::vector<int>* joinTuplesF = &f"<<mapName<<".getValues(sharedVar);\n";
                                *out << ind++ << "for(unsigned i = 0; i < joinTuplesF->size(); i++){\n";
                    ...
                    if(builder->isAggrSetPredicate(aggrSetPred->getPredicateName())){
                        *out << ind++ << "while(joinTuplesU->size()>0){\n";
                            ...
                            *out << --ind << "}\n";
                                    //*out << ind << "std::cout<<\"Propagating from rule: "<<r.getRuleId()<<"\"<<std::endl;\n";
                            // reason contains body, aggr_id and all aggr_set false
                            *out << ind << "propUndefined(factory.getTupleFromInternalID(joinTuplesU->at(0)),false,propagationStack,false,propagatedLiterals,remainingPropagatingLiterals, solver);\n";
                        *out << --ind << "}\n";
                    }else{
                        *out << ind++ << "for(unsigned index=0; index<joinTuplesU->size(); index++){\n";
                            *out << ind << "int itProp = joinTuplesU->at(index);\n";
                            ...
                            *out << --ind << "}\n";
                                    //*out << ind << "std::cout<<\"Propagating from rule: "<<r.getRuleId()<<"\"<<std::endl;\n";
                            // reason contains body, aggr_id and all aggr_set false
                            *out << ind << "propUndefined(factory.getTupleFromInternalID(joinTuplesU->at(index)),false,propagationStack,false,propagatedLiterals,remainingPropagatingLiterals, solver);\n";
                        *out << --ind << "}\n";
                    }
                    ...
                    if(builder->isAggrSetPredicate(aggrSetPred->getPredicateName())){
                        if(aggregateRelation->getAggregate().isSum()){
                            *out << ind++ << "for(unsigned index=0;index<joinTuplesU->size();){\n";
                                *out << ind << "int itProp = joinTuplesU->at(index);\n";
                                *out << ind << "const Tuple* currentJoinTuple = factory.getTupleFromInternalID(joinTuplesU->at(index));\n";
                                *out << ind++ << "if(actualSum[aggrIdIt]+currentJoinTuple->at(0) >= "<<guard<<"){\n";
                            ...
                            *out << --ind << "}\n";
                                    //*out << ind << "std::cout<<\"Propagating from rule: "<<r.getRuleId()<<"\"<<std::endl;\n";
                            if(aggregateRelation->getAggregate().isSum()){
                                *out << ind << "propUndefined(currentJoinTuple,false,propagationStack,true,propagatedLiterals,remainingPropagatingLiterals, solver);\n";
                                *out << --ind << "}else index++;\n";
                            }
                            else
                                *out << ind << "propUndefined(factory.getTupleFromInternalID(joinTuplesU->at(0)),false,propagationStack,true,propagatedLiterals,remainingPropagatingLiterals, solver);\n";
                        *out << --ind << "}\n";
                    }else{
                        *out << ind++ << "for(unsigned index=0; index<joinTuplesU->size(); index++){\n";
                            *out << ind << "const Tuple* currentJoinTuple = factory.getTupleFromInternalID(joinTuplesU->at(index));\n";
                        ...
                        if(aggregateRelation->getAggregate().isSum()){
                            ...
                            *out << ind++ << "if(actualSum[aggrIdIt]+currentJoinTuple->at("<<varIndex<<") >= "<<guard<<"){\n";
                        }
                            *out << ind << "int itProp = joinTuplesU->at(index);\n";
                            *out << ind++ << "if(reasonForLiteral.count(-itProp) == 0 || reasonForLiteral[-itProp].empty()){\n";
                                *out << ind++ << "if(reason.empty()){\n";
                        ...
                        *out << ind++ << "if(startVar > 0){\n";
                            ...
                            *out << --ind << "}else if(head->isUndef()){\n";
                            ind++;
                                ...
                                *out << ind << "propUndefined(head,false,propagationStack,false,propagatedLiterals,remainingPropagatingLiterals, solver);\n";
                            *out << --ind << "}\n";
                        *out << --ind << "}else{\n";
                        ind++;
                            ...
                            if(l->isBoundedLiteral(headVariables)){
                                ...
                                ind++;
                                    *out << ind++ << "if(head != NULL && head->isUndef()){\n";
                                        ...
                                        *out << ind++ << "if(reasonForLiteral.count(-it) == 0 || reasonForLiteral[-it].empty())\n";
                                            *out << ind-- << "reasonForLiteral[-it].insert(startVar);\n";
                                //*out << ind << "std::cout<<\"propagation from rule: "<<r.getRuleId()<<"\"<<std::endl;\n";
                                        *out << ind << "propUndefined(head,false,propagationStack,true,propagatedLiterals,remainingPropagatingLiterals, solver);\n";
                                    *out << --ind << "}\n";
                                *out << --ind << "}\n";
                            }else{
                                *out << ind << "const std::vector<int>* tuples = &p"<<l->getPredicateName()<<"_";
                                ...
                                *out << ind++ << "if(head != NULL && head->isTrue()){\n";
                                    *out << ind++ << "if(tuples->size()==0 && tuplesU->size()==0){\n";
                                        ...
                                        *out << ind << "const std::vector<int>* tuplesF = &f"<<l->getPredicateName()<<"_";
                                        for(unsigned k : boundIndices){
                                            *out << k << "_";
                                        }
                                        *out << ".getValues({"<<boundTerms<<"});\n";
                                ...
                                ind++;
                                    *out << ind++ << "if(tuple->isUndef()){\n";
                                        ...
                                        *out << ind++ << "if(reasonForLiteral.count(it) == 0 || reasonForLiteral[it].empty())\n";
                                            *out << ind-- << "reasonForLiteral[it].insert(startVar);\n";
                                //*out << ind << "std::cout<<\"propagation from rule: "<<r.getRuleId()<<"\"<<std::endl;\n";
                                        *out << ind << "propUndefined(tuple,false,propagationStack,false,propagatedLiterals,remainingPropagatingLiterals, solver);\n";
                                    *out << --ind << "}\n";
                                *out << --ind << "}\n";
                            *out << --ind << "}else{\n";
                            ind++;
                                ...
                                ind++;
                                    *out << ind++ << "if(tuple->isUndef()){\n";
                                        ...
                                        *out << ind++ << "if(reasonForLiteral.count(-it) == 0 || reasonForLiteral[-it].empty())\n";
                                            *out << ind-- << "reasonForLiteral[-it].insert(startVar);\n";
                                            //*out << ind << "std::cout<<\"propagation from rule: "<<r.getRuleId()<<"\"<<std::endl;\n";
                                        *out << ind << "propUndefined(tuple,false,propagationStack,true,propagatedLiterals,remainingPropagatingLiterals, solver);\n";
                                    *out << --ind << "}\n";
                                *out << --ind << "}\n";
                            *out << --ind << "}\n";
                        }else{
                            ...
                            *out << --ind << "}else{\n";
                            ind++;
                                ...
                                *out << --ind << "}else{\n";
                                ...
                                //*out << ind << "std::cout<<\"propagation from rule: "<<r.getRuleId()<<"\"<<std::endl;\n";
                                            *out << ind << "propUndefined(factory.getTupleFromInternalID(tuplesU->back()),false,propagationStack,true,propagatedLiterals,remainingPropagatingLiterals, solver);\n";
        ...
        if(!fromStarter){
            *out << ind++ << "{\n";
            ...
            const aspc::Atom* head = &r.getHead()[0];
            {
                ...
                std::unordered_set<std::string> boundVariables;
                *out << ind << "const std::vector<int>* trueHeads = &p"<<head->getPredicateName()<<"_.getValues({});\n";
            ...
            }
            {
                ...
                std::unordered_set<std::string> boundVariables;
                *out << ind << "const std::vector<int>* undefHeads = &u"<<head->getPredicateName()<<"_.getValues({});\n";
            ...
            }
            {
                //print false head propagation
                std::unordered_set<std::string> boundVariables;
                *out << ind << "const std::vector<int>* falseHeads = &f"<<head->getPredicateName()<<"_.getValues({});\n";
                    ...
                    if(l->isBoundedLiteral(boundVariables)){
                        ...
                        *out << ind++ << "if(tuples->size() == 0){\n";
                            ...
                            *out << ind++ << "for(unsigned j = 0; j < tuplesU->size();){\n";
                            //*out << ind << "std::cout<<\"propagation from rule "<<r.getRuleId()<<"\"<<std::endl;\n";
                                *out << ind << "propUndefined(factory.getTupleFromInternalID(tuplesU->at(j)),false,propagationStack,true,propagatedLiterals,remainingPropagatingLiterals, solver);\n";
                            *out << --ind << "}\n";
        ...
        *out << ind++ << "{\n";
        ...
        for(unsigned i = 0; i<starters.size(); i++){
            ...
            *out << ind++ << "if(tupleU != NULL){\n";
                ...
                *out << ind << "bool conflict = propUndefined(tupleU,tupleUNegated,propagationStack,true,propagatedLiterals,remainingPropagatingLiterals, solver);\n";
                // *out << ind << "UnorderedSet<int> rrrrrr;\n";
                // *out << ind << "std::unordered_set<int> visitedLiteralssssssss;\n";
                // *out << ind << "int it = tupleU->getId();\n";
