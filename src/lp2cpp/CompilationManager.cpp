...
void CompilationManager::generateStratifiedCompilableProgram(aspc::Program & program, AspCore2ProgramBuilder* builder) {
    ...
    *out << ind << "\n";
    *out << ind << "const std::vector<unsigned> EMPTY_TUPLES;\n";
                    ...
                    if(aggrId->getAriety() == 0){
                        ...
                        *out << ind << "possibleSum[itAggrId]-=tuple->at("<<sumVar<<");\n";
                        ...
                        for(std::string sign : {"p","u","f"}){
                            *out << ind++ << "{\n";
                                *out << ind << "const std::vector<unsigned>* aggrIds = &"<<sign<<mapName<<".getValues({"<<terms<<"});\n";
                    ...
                    if(auxToBody.count(it->second.name)!=0){
                            ...
                            if(selectedIneq != NULL){
                                ...
                                if(selectedLit!=NULL){
                                    ...
                                    if(selectedLit->isBoundedLiteral(boundVariables)){
                                        ...
                                        *out << ind << "const std::vector<unsigned>* tuples = &p"<<selectedLit->getPredicateName()<<"_";
                                        ...
                                        *out << ind << "const std::vector<unsigned>* tuplesU = &u"<<selectedLit->getPredicateName()<<"_";
                                    ...
                                    if(!r.isConstraint() && r.containsAggregate()){
                                        ...
                                        if(aggrSetLiteral->getPredicateName() == aux.getPredicateName()&& aggrSetToAuxVal.count(aux.getPredicateName())!=0){
                                            ...
                                            if(distinctBody.count(bodyLiteral->getPredicateName())==0){
                                                ...
                                                storePossibleSum(aggrSetToAuxVal[aux.getPredicateName()],"aux",sharedIndexWithBody,0);
                                            }
                                        }                                        
                                    }
                                }
                                                    ...
                                                    if(!r.isConstraint() && r.containsAggregate()){
                                                        ...
                                                        if(aggrSetLiteral->getPredicateName() == head->getPredicateName() && aggrSetToAuxVal.count(head->getPredicateName())!=0){
                                                            ...
                                                            if(distinctBody.count(bodyLiteral->getPredicateName())==0){
                                                                ...
                                                                storePossibleSum(aggrSetToAuxVal[head->getPredicateName()],"head",sharedIndexWithBody,0);
                                                            }
                                                        }                                        
                                                    }
                                                }
                        ...
                        *out << ind++ << "{\n";
                            ...
                            *out << ind++ << "for(unsigned i = 0; i < tuplesU->size(); i++){\n";
                                *out << ind << "Tuple * tuple = factory.find(tuplesU->at(i));\n";
                                        ...
                                        if(!r.isConstraint() && r.containsAggregate()){
                                            ...
                                            if(aggrSetLiteral->getPredicateName() == it->second.name && aggrSetToAuxVal.count(it->second.name)!=0){
                                                ...
                                                if(distinctBody.count(bodyLiteral->getPredicateName())==0){
                                                    ...
                                                    storePossibleSum(aggrSetToAuxVal[it->second.name],"tuple",sharedIndexWithBody,0);
                                                }
                                            }                                        
                                        }
                                    }
                        ...
                        for(const aspc::Rule& r : program.getRules()){
                            if(!r.isConstraint() && r.getHead()[0].getPredicateName()==it->second.name && !r.containsAggregate()){
                                if(r.getBodyLiterals().size()==1 && auxToBody.count(r.getBodyLiterals()[0].getPredicateName())==0){
                                    ...
                                    *out << ind++ << "{\n";
                                        *out << ind << "const std::vector<unsigned>* tuples = &p"<<bodyLit->getPredicateName()<<"_.getValues({});\n";
                                        *out << ind << "const std::vector<unsigned>* tuplesU = &u"<<bodyLit->getPredicateName()<<"_.getValues({});\n";
                                        *out << ind++ << "for(unsigned i=0; i<tuples->size()+tuplesU->size(); i++){\n";
                                            *out << ind << "const Tuple* tuple = NULL;\n";
                                            *out << ind++ << "if(i<tuples->size()){\n";
                                                *out << ind << "tuple=factory.getTupleFromInternalID(tuples->at(i));\n";
        ...
        for(const auto& aggrSetPred : aggrSetToRule){
            for(unsigned ruleId : aggrSetPred.second){
                ...
                if(aggregateRelation->getAggregate().isSum()){
                    ...
                    *out << ind++ << "{\n";
                        *out << ind << "const std::vector<unsigned>* aggregateIds = &u"<<aggrId->getPredicateName()<<"_.getValues({});\n";
                        *out << ind++ << "for(unsigned i=0;i<aggregateIds->size();i++){\n";
                            ...
                            *out << ind << "const std::vector<unsigned>* aggrSetTuples = &u"<<aggrSetPred.first<<"_";
                                        ...
                                        for(unsigned formulaIndex: orderedBodyFormulas){
                                            ...
                                            if(!f->isLiteral() && !f->containsAggregate()){
                                                ...
                                                if(f->isBoundedLiteral(boundVariables)){
                                                    ...
                                                    *out << ind << "const std::vector<unsigned>* tuples = &p"<<l->getPredicateName()<<"_";
                                                    std::string boundTerms="";
    ...
    for(unsigned i : body){
        ...
        if(f->isLiteral()){
            ...
            if(lit->isBoundedLiteral(boundVars)){
                ...
                *out << ind << "const std::vector<unsigned>* tuples = &p"<<lit->getPredicateName()<<"_";
                ...
                *out << "});\n";
                *out << ind << "const std::vector<unsigned>* tuplesU = &EMPTY_TUPLES;\n";
                if(searchOneUndef)
...
void CompilationManager::compileEagerRuleWithAggregate(const aspc::Rule& r,bool fromStarter){
    ...
    if(fromStarter){
        {
            ...
            *out << ind++ << "if(starter.getPredicateName() == &_"<<aggrIdAtom->getPredicateName()<<"){\n";
            ...
            for(unsigned i = 0; i<aggrIdAtom->getAriety(); i++){
                ...
                *out << ind << "const std::vector<unsigned>* tuples = &p"<<mapName<<".getValues(sharedVar);\n";
                *out << ind << "const std::vector<unsigned>* tuplesU = &u"<<mapName<<".getValues(sharedVar);\n";
                string bodyTerms = "";
                if(bodyPred!=NULL){
                        ...
                        if(builder->isAggrSetPredicate(aggrSetPred->getPredicateName())){
                            *out << ind++ << "for(int index=0; index<tuplesU->size();){\n";
                                ...
                                *out << ind++ << "if(actualSum[uStartVar]+currentTuple->at(0) >= "<<guard<<"){\n";
                                    ...
                                    *out << ind << "propUndefined(currentTuple,false,propagationStack,true,propagatedLiterals,remainingPropagatingLiterals);\n";
                                *out << --ind << "}else index++;\n";
                            *out << --ind << "}\n";
                        }else{
                            std::string firstAggrVar = aggregateRelation->getAggregate().getAggregateVariables()[0];
                            ...
                            *out << ind << "reason.push_back(startVar);\n";
                            if(builder->isAggrSetPredicate(aggrSetPred->getPredicateName())){
                                *out << ind++ << "while(!tuplesU->empty()){\n";
                                    ...
                                    *out << --ind << "}\n";
                                    //*out << ind << "std::cout<<\"Propagating from rule: "<<r.getRuleId()<<"\"<<std::endl;\n";
                                    *out << ind << "propUndefined(factory.getTupleFromInternalID(tuplesU->at(0)),false,propagationStack,true,propagatedLiterals,remainingPropagatingLiterals);\n";
                                *out << --ind << "}\n";
                            }else{
                                *out << ind++ << "for(unsigned i =0; i<tuplesU->size(); i++){\n";
                                    ...
                                    *out << --ind << "}\n";
                                    //*out << ind << "std::cout<<\"Propagating from rule: "<<r.getRuleId()<<"\"<<std::endl;\n";
                                    *out << ind << "propUndefined(factory.getTupleFromInternalID(tuplesU->at(i)),false,propagationStack,true,propagatedLiterals,remainingPropagatingLiterals);\n";
                                *out << --ind << "}\n";
                            }
                ...
                *out << --ind << "}else{\n";
                ind++;
                    if(aggregateRelation->getAggregate().isSum())
                        ...
                        *out << ind << "const std::vector<unsigned>* tuplesF = &f"<<mapName<<".getValues(sharedVar);\n";
                        ...
                        if(builder->isAggrSetPredicate(aggrSetPred->getPredicateName())){
                            *out << ind++ << "for(unsigned index=0;index<tuplesU->size();){\n";
                                ...
                                *out << ind++ << "if(actualSum[uStartVar]+possibleSum[uStartVar]-currentTuple->at(0) < "<<guard<<"){\n";
                                    ...
                                    *out << ind << "propUndefined(currentTuple,false,propagationStack,false,propagatedLiterals,remainingPropagatingLiterals);\n";
                                *out << --ind << "}else index++;\n";
                            *out << --ind << "}\n";
                        }else{
                            std::string firstAggrVar = aggregateRelation->getAggregate().getAggregateVariables()[0];
                        ...
                        if(builder->isAggrSetPredicate(aggrSetPred->getPredicateName())){
                            *out << ind++ << "while(tuplesU->size()>0){\n";
                                ...
                                *out << --ind << "}\n";
                                    //*out << ind << "std::cout<<\"Propagating from rule: "<<r.getRuleId()<<"\"<<std::endl;\n";
                                // reason contains aggr_id and all aggr_set false
                                *out << ind << "propUndefined(factory.getTupleFromInternalID(tuplesU->at(0)),false,propagationStack,false,propagatedLiterals,remainingPropagatingLiterals);\n";
                            *out << --ind << "}\n";
                        }else{
                            *out << ind++ << "for(unsigned index=0;index<tuplesU->size();index++){\n";
                                *out << ind << "int itProp = tuplesU->at(index);\n";
                                ...
                                *out << --ind << "}\n";
                                    //*out << ind << "std::cout<<\"Propagating from rule: "<<r.getRuleId()<<"\"<<std::endl;\n";
                                // reason contains aggr_id and all aggr_set false
                                *out << ind << "propUndefined(factory.getTupleFromInternalID(tuplesU->at(index)),false,propagationStack,false,propagatedLiterals,remainingPropagatingLiterals);\n";
                            *out << --ind << "}\n";
                        }
                        *out << --ind << "}\n";
                    }
    ...
    }
    {
        ...
        if(bodyPred!=NULL){
            ...
            *out << ind << "const std::vector<unsigned>* tuples = &p"<<aggrIdAtom->getPredicateName()<<"_.getValues({});\n";
            *out << ind << "const std::vector<unsigned>* tuplesU = &u"<<aggrIdAtom->getPredicateName()<<"_.getValues({});\n";
            *out << ind << "const std::vector<unsigned>* tuplesF = &f"<<aggrIdAtom->getPredicateName()<<"_.getValues({});\n";
            ...
            *out << ind++ << "for(unsigned i = 0; i<tuples->size(); i++){\n";
            {
                ...
                *out << ind << "const std::vector<unsigned>* joinTuples = &p"<<mapName<<".getValues(sharedVar);\n";
                *out << ind << "const std::vector<unsigned>* joinTuplesU = &u"<<mapName<<".getValues(sharedVar);\n";
                ...
                if(aggregateRelation->getAggregate().isSum())
                    ...
                    *out << ind++ << "if(joinTuples->size() + joinTuplesU->size() < "<<guard<<"){\n";
                    ...
                    if(fromStarter){
                        ...
                        *out << ind << "const std::vector<unsigned>* joinTuplesF = &f"<<mapName<<".getValues(sharedVar);\n";
                    ...
                    if(builder->isAggrSetPredicate(aggrSetPred->getPredicateName())){
                        *out << ind++ << "for(unsigned index=0; index<joinTuplesU->size();){\n";
                            ...
                            *out << --ind << "}\n";
                                    //*out << ind << "std::cout<<\"Propagating from rule: "<<r.getRuleId()<<"\"<<std::endl;\n";
                            // reason contains aggr_id and all aggr_set false
                            *out << ind << "propUndefined(currentJoinTuple,false,propagationStack,false,propagatedLiterals,remainingPropagatingLiterals);\n";
                        *out << --ind << "}\n";
                    }else{
                        std::string firstAggrVar = aggregateRelation->getAggregate().getAggregateVariables()[0];
                        unsigned varIndex = 0;
                    ...
                    if(builder->isAggrSetPredicate(aggrSetPred->getPredicateName())){
                        *out << ind++ << "while(joinTuplesU->size()>0){\n";
                            ...
                            *out << --ind << "}\n";
                                    //*out << ind << "std::cout<<\"Propagating from rule: "<<r.getRuleId()<<"\"<<std::endl;\n";
                            // reason contains body, aggr_id and all aggr_set false
                            *out << ind << "propUndefined(factory.getTupleFromInternalID(joinTuplesU->at(0)),false,propagationStack,false,propagatedLiterals,remainingPropagatingLiterals);\n";
                        *out << --ind << "}\n";
                    }else{
                        *out << ind++ << "for(unsigned index=0; index<joinTuplesU->size(); index++){\n";
                            *out << ind << "int itProp = joinTuplesU->at(index);\n";
                            ...
                            *out << --ind << "}\n";
                                    //*out << ind << "std::cout<<\"Propagating from rule: "<<r.getRuleId()<<"\"<<std::endl;\n";
                            // reason contains body, aggr_id and all aggr_set false
                            *out << ind << "propUndefined(factory.getTupleFromInternalID(joinTuplesU->at(index)),false,propagationStack,false,propagatedLiterals,remainingPropagatingLiterals);\n";
                        *out << --ind << "}\n";
                    }
                    ...
                    if(builder->isAggrSetPredicate(aggrSetPred->getPredicateName())){
                        if(aggregateRelation->getAggregate().isSum()){
                            ...
                            *out << --ind << "}\n";
                                    //*out << ind << "std::cout<<\"Propagating from rule: "<<r.getRuleId()<<"\"<<std::endl;\n";
                            if(aggregateRelation->getAggregate().isSum()){
                                *out << ind << "propUndefined(currentJoinTuple,false,propagationStack,true,propagatedLiterals,remainingPropagatingLiterals);\n";
                                *out << --ind << "}else index++;\n";
                            }
                            else
                                *out << ind << "propUndefined(factory.getTupleFromInternalID(joinTuplesU->at(0)),false,propagationStack,true,propagatedLiterals,remainingPropagatingLiterals);\n";
                        *out << --ind << "}\n";
                    }else{
                        *out << ind++ << "for(unsigned index=0; index<joinTuplesU->size(); index++){\n";
                            *out << ind << "const Tuple* currentJoinTuple = factory.getTupleFromInternalID(joinTuplesU->at(index));\n";
                        ...
                        *out << ind++ << "if(startVar > 0){\n";
                            ...
                            *out << --ind << "}else if(head->isUndef()){\n";
                            ind++;
                                ...
                                *out << ind << "propUndefined(head,false,propagationStack,false,propagatedLiterals,remainingPropagatingLiterals);\n";
                            *out << --ind << "}\n";
                        *out << --ind << "}else{\n";
                        ind++;
                            ...
                            *out << "}, &_"<<head->getPredicateName()<<");\n";
                            if(l->isBoundedLiteral(headVariables)){
                                ...
                                ind++;
                                    *out << ind++ << "if(head != NULL && head->isUndef()){\n";
                                        ...
                                        *out << ind++ << "if(reasonForLiteral.count(-it) == 0 || reasonForLiteral[-it].empty())\n";
                                            *out << ind-- << "reasonForLiteral[-it].insert(startVar);\n";
                                //*out << ind << "std::cout<<\"propagation from rule: "<<r.getRuleId()<<"\"<<std::endl;\n";
                                        *out << ind << "propUndefined(head,false,propagationStack,true,propagatedLiterals,remainingPropagatingLiterals);\n";
                                    *out << --ind << "}\n";
                                *out << --ind << "}\n";
                            }else{
                                *out << ind << "const std::vector<unsigned>* tuples = &p"<<l->getPredicateName()<<"_";
                                ...
                                *out << ".getValues({"<<boundTerms<<"});\n";
                                *out << ind << "const std::vector<unsigned>* tuplesU = &u"<<l->getPredicateName()<<"_";
                                for(unsigned k : boundIndices){
                                    *out << k << "_";
                                }
                                *out << ".getValues({"<<boundTerms<<"});\n";
                                *out << ind++ << "if(head != NULL && head->isTrue()){\n";
                                    *out << ind++ << "if(tuples->size()==0 && tuplesU->size()==0){\n";
                                        ...
                                        *out << ind << "const std::vector<unsigned>* tuplesF = &f"<<l->getPredicateName()<<"_";
                                        for(unsigned k : boundIndices){
                                            *out << k << "_";
                                        }
                                        *out << ".getValues({"<<boundTerms<<"});\n";
                                ...
                                ind++;
                                    *out << ind++ << "if(tuple->isUndef()){\n";
                                        ...
                                        *out << ind++ << "if(reasonForLiteral.count(it) == 0 || reasonForLiteral[it].empty())\n";
                                            *out << ind-- << "reasonForLiteral[it].insert(startVar);\n";
                                //*out << ind << "std::cout<<\"propagation from rule: "<<r.getRuleId()<<"\"<<std::endl;\n";
                                        *out << ind << "propUndefined(tuple,false,propagationStack,false,propagatedLiterals,remainingPropagatingLiterals);\n";
                                    *out << --ind << "}\n";
                                *out << --ind << "}\n";
                            *out << --ind << "}else{\n";
                            ind++;
                                ...
                                ind++;
                                    *out << ind++ << "if(tuple->isUndef()){\n";
                                        ...
                                        *out << ind++ << "if(reasonForLiteral.count(-it) == 0 || reasonForLiteral[-it].empty())\n";
                                            *out << ind-- << "reasonForLiteral[-it].insert(startVar);\n";
                                            //*out << ind << "std::cout<<\"propagation from rule: "<<r.getRuleId()<<"\"<<std::endl;\n";
                                        *out << ind << "propUndefined(tuple,false,propagationStack,true,propagatedLiterals,remainingPropagatingLiterals);\n";
                                    *out << --ind << "}\n";
                                *out << --ind << "}\n";
                            *out << --ind << "}\n";
                        }else{
                            ...
                            *out << --ind << "}else{\n";
                            ind++;
                                ...
                                *out << --ind << "}else{\n";
                                ...
                                //*out << ind << "std::cout<<\"propagation from rule: "<<r.getRuleId()<<"\"<<std::endl;\n";
                                            *out << ind << "propUndefined(factory.getTupleFromInternalID(tuplesU->back()),false,propagationStack,true,propagatedLiterals,remainingPropagatingLiterals);\n";
        ...
        if(!fromStarter){
            *out << ind++ << "{\n";
            ...
            const aspc::Atom* head = &r.getHead()[0];
            {
                ...
                std::unordered_set<std::string> boundVariables;
                *out << ind << "const std::vector<unsigned>* trueHeads = &p"<<head->getPredicateName()<<"_.getValues({});\n";
            ...
            }
            {
                ...
                std::unordered_set<std::string> boundVariables;
                *out << ind << "const std::vector<unsigned>* undefHeads = &u"<<head->getPredicateName()<<"_.getValues({});\n";
            ...
            }
            {
                //print false head propagation
                std::unordered_set<std::string> boundVariables;
                *out << ind << "const std::vector<unsigned>* falseHeads = &f"<<head->getPredicateName()<<"_.getValues({});\n";
                    ...
                    if(l->isBoundedLiteral(boundVariables)){
                        ...
                        *out << ind++ << "if(tuples->size() == 0){\n";
                            ...
                            *out << ind++ << "for(unsigned j = 0; j < tuplesU->size();){\n";
                            //*out << ind << "std::cout<<\"propagation from rule "<<r.getRuleId()<<"\"<<std::endl;\n";
                                *out << ind << "propUndefined(factory.getTupleFromInternalID(tuplesU->at(j)),false,propagationStack,true,propagatedLiterals,remainingPropagatingLiterals);\n";
                            *out << --ind << "}\n";
        ...
        *out << ind++ << "{\n";
        ...
        for(unsigned i = 0; i<starters.size(); i++){
            ...
            *out << ind++ << "if(tupleU != NULL){\n";
                ...
                *out << ind << "bool conflict = propUndefined(tupleU,tupleUNegated,propagationStack,true,propagatedLiterals,remainingPropagatingLiterals);\n";
                // *out << ind << "UnorderedSet<int> rrrrrr;\n";
                // *out << ind << "std::unordered_set<int> visitedLiteralssssssss;\n";
                // *out << ind << "int it = tupleU->getId();\n";
