...
void CompilationManager::declareDataStructureForAggregate(const aspc::Rule& r,const std::set< pair<std::string, unsigned> >& aggregatePredicates){
    ...
    for(const aspc::ArithmeticRelationWithAggregate& aggr : r.getArithmeticRelationsWithAggregate()){
        ...
        declareAuxMap("_"+joinTupleName+sharedVarsIndexToString+aggregateVarsIndex,sharedAndAggrVarIndex,joinTupleName,false);
        ...
        for(const aspc::Literal& l : aggr.getAggregate().getAggregateLiterals()){
            ...
            declareAuxMap("_"+joinTupleName+sharedVarsIndexToString+lit_indeces,total_indeces,joinTupleName,false);
        ...
        for(std::vector<unsigned>& indexes : aggregateLiteralIndexes){
            ...
            if(sharedVariables!="")
                declareAuxMap("_"+joinTupleName+literalTermIndex+sharedVarsIndexToString,indexes,joinTupleName,false);
            for(unsigned v : aggregateVariablesIndex[aggrIdentifier]){
                indexes.push_back(v);
            }
            declareAuxMap("_"+joinTupleName+literalTermIndex+sharedVarsIndexToString+aggregateVarsIndex,indexes,joinTupleName,false);
        ...
        if(sharedVariables!=""){
            ...
            for(const aspc::ArithmeticRelationWithAggregate& aggr : r.getArithmeticRelationsWithAggregate()){
                ...
                for(const aspc::Literal& aggrLit : aggr.getAggregate().getAggregateLiterals()){
                    ...
                    declareAuxMap(auxMapName,indexes,aggrLit.getPredicateName(),declareFalseAuxMap);
                    ...
                    for(const aspc::Literal& buildAggrLiteral : aggr.getAggregate().getAggregateLiterals()){
                        if(bodyLiteralIndex!=literalIndex){
                            ...
                            //std::cout<<"Declare "<<buildAuxMapName<<std::endl;
                            declareAuxMap(buildAuxMapName,buildindexes,buildAggrLiteral.getPredicateName(),declareFalseAuxMap);
...
void CompilationManager::generateStratifiedCompilableProgram(aspc::Program & program, AspCore2ProgramBuilder* builder) {
    ...
    *out << ind << "std::unordered_map<std::vector<int>, unsigned int,TuplesHash> actualSum["<<aggregateToStructure.size()<<"];\n";
    *out << ind << "std::unordered_map<std::vector<int>, unsigned int,TuplesHash> possibleSum["<<aggregateToStructure.size()<<"];\n";
                                    ...
                                    *out << ind++ << "for(int i_"<<buildIndex<<"=0;i_"<<buildIndex<<"<tuples"<<buildIndex<<"->size()+tuplesU"<<buildIndex<<"->size();i_"<<buildIndex<<"++){\n";
                                    closingPars++;
                                        ...
                                        *out << ind++ << "if(i_"<<buildIndex<<"<tuples"<<buildIndex<<"->size())\n";
                                            *out << ind-- << "tuple"<<buildIndex<<"=tuples"<<buildIndex<<"->at(i_"<<buildIndex<<");\n";
                                        *out << ind++ << "else{\n";
                                            *out << ind << "tuple"<<buildIndex<<"=tuplesU"<<buildIndex<<"->at(i_"<<buildIndex<<"-tuples"<<buildIndex<<"->size());\n";
                                            *out << ind << "undef"<<buildIndex<<"=true;\n";
                                        *out << --ind << "}\n";
...
//                        *out << ind << "std::cout<<\"Join true tuple built\"<<std::endl;\n";//DEBUG_PRINTING
                        *out << ind++ << "if(w"<<aggr->getJoinTupleName()<<".find(t)==NULL){\n";
//                            *out << ind << "std::cout<<\"New true\"<<std::endl;\n";//DEBUG_PRINTING
                            *out << ind++ << "if(u"<< aggr->getJoinTupleName() <<".find(t))\n";
                            *out << ind-- << "u" << aggr->getJoinTupleName() <<".erase(t);\n";
                            ...
                            *out << ind << "const auto& insertResult = w" << aggr->getJoinTupleName() <<".insert(Tuple(t));\n";
                            *out << ind++ << "if (insertResult.second) {\n";
//                                *out << ind << "std::cout<<\"New true saved\"<<std::endl;\n";//DEBUG_PRINTING
                                *out << ind++ << "for(AuxMap* auxMap : predicateToAuxiliaryMaps[&_"<<aggr->getJoinTupleName()<<"]){\n";
                                    *out << ind << "auxMap -> insert2(*insertResult.first);\n";
                                *out << --ind << "}\n";
                                // *out << ind << "std::cout<<\"insert in true \"<<w"<<aggr->getJoinTupleName()<<".getTuples().size()<<std::endl;\n";
                        ...
                        //--------------------------END SAVING NEW TRUE JOIN TUPLE--------------------------
                            ...
                            for(const auto& aggrIdentifier: pair.second){
                                ...
                                *out << ind++ << "{\n";
                                    int closingLocalPars=0;
                                    

                                        //--------------------------UPDATE SHARED VARIABLE STRUCTURE--------------------------
                                        if(sharedVariablesMap[key]!=""){
                                            //*out << ind << "const auto& findResult = w"<<aggregate->getJoinTupleName()<<".find(Tuple(t));\n";
                                            // *out << ind++ << "if(sharedVariables_"<<ruleId<<"_ToAggregate_"<<aggrIndex<<".count({"<<sharedVariablesMap[key]<<"})>0)\n";
                                            //     *out << ind-- << "sharedVariables_"<<ruleId<<"_ToAggregate_"<<aggrIndex<<"[{"<<sharedVariablesMap[key]<<"}]->first->insert2(*insertResult.first);\n";
                                        }
                                        //--------------------------END UPDATE SHARED VARIABLE STRUCTURE--------------------------
                                        ...
                                        *out << "});\n";
                                            ...
                                            *out << ind << "auto& undefSet = undefAggrVars["<<aggregateToStructure[aggregate->getJoinTupleName()+sharedVariablesMap[key]+aggregate->getAggrVarAsString()]<<"][{"<<sharedVariablesMap[key]<<"}];\n";
                                            ...
                                            // *out << ind++ << "if(u_"<<aggregate->getJoinTupleName()<<sharedVarIndex<<aggrVarProj<<".getValues({"<<sharedVariablesMap[key]<<comma<<aggrVarTuple<<"}).size()>0){\n";
                                            ...
                                            *out << --ind << "}\n";
                                        // *out << ind++ << "if(p_"<<aggregate->getJoinTupleName()<<sharedVarIndex<<aggrVarProj<<".getValues({"<<sharedVariablesMap[key]<<comma<<aggrVarTuple<<"}).size()>0){\n";
                                        ...
                                        *out << --ind << "}\n";
//                                        *out << ind << "std::cout<<\"AGGREGATE VARIABLE STRUCTURE UPDATED\"<<std::endl;\n";//DEBUG_PRINTING
    ...
    //                                                        *out << ind << "std::cout<<\"Adding on lit true\"<<std::endl;\n";//DEBUG_PRINTING
                                                            *out << ind << "positiveAggrReason["<<aggregateToStructure[aggregate->getJoinTupleName()+sharedVariablesMap[key]+aggregate->getAggrVarAsString()]<<"][{"<<sharedVariablesMap[key]<<"}].insert(it->second"<<sign<<");\n";
                                                        *out << --ind << "}\n";
                                                    *out << --ind << "}\n";
                                                }else{
                                                    *out << ind << "positiveAggrReason["<<aggregateToStructure[aggregate->getJoinTupleName()+sharedVariablesMap[key]+aggregate->getAggrVarAsString()]<<"][{"<<sharedVariablesMap[key]<<"}].insert(var);\n";
                                                }
                                                buildIndex++;
                                            }
                                        }
//                                        *out << ind << "std::cout<<\"POSITIVE REASON STRUCTURE UPDATED\"<<std::endl;\n";//DEBUG_PRINTING
                                        //--------------------------END UPDATE POSITIVE REASON STRUCTURE--------------------------
                                        

                                    while(closingLocalPars>0){
                                        *out << --ind << "}\n";  
                                        closingLocalPars--; 
                                    }                                 
                                *out << --ind << "}\n";
                        ...
                        *out << --ind << "}else{\n";
                        ind++;
                            //--------------------------SAVING NEW UNDEF--------------------------
                            *out << ind << "Tuple t({"<<jointuple.substr(0,jointuple.length()-1)<<"},&_"<<aggr->getJoinTupleName()<<");\n";
                            *out << ind++ << "if(u"<<aggr->getJoinTupleName()<<".find(t)==NULL){\n";
                    ...
                    *out << --ind << "}else{\n";
                        ...
                        *out << ind++ << "while(!tuplesU.empty()){\n";
                            ...
                            *out << ind++ << "{\n";
                            ...
                            if(aggregate->isNegated()){
                                *out << ind << "negativeAggrReason["<<aggregateToStructure[aggregate->getJoinTupleName()+sharedVariablesMap[key]+aggregate->getAggrVarAsString()]<<"][{"<<sharedVariablesMap[key]<<"}].insert(var);\n";
                            }
    ...
    // *out << ind << "std::cout<<\"Possible sum for 1: \"<<possibleSum[0][{1}]<<std::endl;\n";
    // *out << ind << "std::cout<<\"Possible sum for 2: \"<<possibleSum[0][{2}]<<std::endl;\n";
    // *out << ind << "std::cout<<\"Possible sum for 3: \"<<possibleSum[0][{3}]<<std::endl;\n";
    // *out << ind << "std::cout<<\"-----------------------------------------\"<<std::endl;\n";
    // *out << ind << "std::cout<<\"Possible sum for 1: \"<<actualSum[0][{1}]<<std::endl;\n";
    // *out << ind << "std::cout<<\"Possible sum for 2: \"<<actualSum[0][{2}]<<std::endl;\n";
    // *out << ind << "std::cout<<\"Possible sum for 3: \"<<actualSum[0][{3}]<<std::endl;\n";
    // *out << ind << "std::cout<<\"-----------------------------------------\"<<std::endl;\n";
    // *out << ind << "std::cout<<\"Possible joinTupleSize for 1: \"<<undefAggrVars[0][{1}].size()<<std::endl;\n";
    // *out << ind << "std::cout<<\"Possible joinTupleSize for 2: \"<<undefAggrVars[0][{2}].size()<<std::endl;\n";
    // *out << ind << "std::cout<<\"Possible joinTupleSize for 3: \"<<undefAggrVars[0][{3}].size()<<std::endl;\n";
    // *out << ind << "std::cout<<\"End on lit true\"<<std::endl;\n";
    *out << --ind << "}\n";
    ...
    *out << ind++ << "inline void Executor::onLiteralUndef(int var) {\n";
                            ...
                            //--------------------------END INSERT UNDEF--------------------------
                                ...
                                for(const auto& aggrIdentifier: pair.second){
                                    ...
                                    *out << --ind << "}\n";
                                    ...
                                    //--------------------------UPDATE POSITIVE RESON STRUCTURE--------------------------
                                    if(!aggregate->isNegated()){
                                        int buildIndex=0;
                                        int totalAriety=0;
                                        for(const aspc::Literal& li : aggregate->getAggregate().getAggregateLiterals()){
                                            std::string buildLitTerms;
                                            for(int i=0;i<li.getAriety();i++){
                                                if(i>0)
                                                    buildLitTerms += ",";
                                                buildLitTerms += "t["+std::to_string(i+totalAriety)+"]";
                                            }
                                            std::string sharedVarProj="";
                                            std::string sharedVarTerms=sharedVariablesMap[key]!="" ? ","+sharedVariablesMap[key] : "";
                                            if(sharedVariablesMap[key]!=""){
                                                for(unsigned v : sharedVariablesIndexesMap[key]){
                                                    sharedVarProj+=std::to_string(v)+"_";
                                                }
                                            }
                                            *out << ind++ << "if(p_"<<aggregate->getJoinTupleName()<<aggregate->getAggrLitProj(buildIndex)<<sharedVarProj<<".getValues({"<<buildLitTerms<<sharedVarTerms<<"}).size() == 0){\n";
                                                *out << ind << "const auto & it = tupleToVar.find(Tuple({"<<buildLitTerms<<"},&_"<<li.getPredicateName()<<"));\n";
                                                *out << ind++ << "if(it != tupleToVar.end()) {\n";
                                                    std::string sign = li.isNegated() ? "-1*":"";
                                                    // std::string s = li.isNegated() ? "-":"";
                                                    // *out << ind << "Tuple removing({"<<buildLitTerms<<"},&_"<<li.getPredicateName()<<");\n";
                                                    // *out << ind << "removing.print();\n";
                                                    // *out << ind << "std::cout<<\"Removing from positive reason "<<s<<"\"<<it->second<<std::endl;\n";
                                                    *out << ind << "positiveAggrReason["<<aggregateToStructure[aggregate->getJoinTupleName()+sharedVariablesMap[key]+aggregate->getAggrVarAsString()]<<"][{"<<sharedVariablesMap[key]<<"}].erase("<<sign<<"it->second);\n";
                                                *out << --ind << "}\n";
                                            *out << --ind << "}\n";
                                            buildIndex++;
                                            totalAriety+=li.getAriety();
                                        }
                                    }
                                    //--------------------------END UPDATE POSITIVE RESON STRUCTURE--------------------------
                                    *out << --ind << "}\n";
                                }
                                *out << --ind << "}\n";
    ...
    // *out << ind << "std::cout<<\"Possible joinTupleSize for 2: \"<<undefAggrVars[0][{2}].size()<<std::endl;\n";
    // *out << ind << "std::cout<<\"Possible joinTupleSize for 3: \"<<undefAggrVars[0][{3}].size()<<std::endl;\n";
    // *out << ind++ << "for(auto pair : undefAggrVars[0]){\n";
    //     *out << ind << "std::cout<<\"--------------------------------\"<<std::endl;\n";
    //     *out << ind++ << "for(int i=0;i<pair.first.size();i++)\n";
    //         *out << ind-- << "std::cout<<pair.first[i]<<\" \";\n";
    //     *out << ind << "std::cout<<std::endl<<pair.second.size()<<std::endl<<\"--------------------------------\"<<std::endl;\n";
    // *out << --ind << "}\n";
    // *out << ind << "std::cout<<\"End on lit undef\"<<std::endl;\n";
    // *out << ind << "std::cout<<ulives_XDX_YDY_diff_DX_DY_XDXEQXPLUSDX_YDYEQYPLUSDY_X_Y_.getTuples().size()<<std::endl;\n";
    *out << --ind << "}\n";
...
void CompilationManager::propAggr(const aspc::ArithmeticRelationWithAggregate* aggregateRelation,std::string& aggregateIdentifier,bool withReason,std::string op){
    ...
    if(aggregateRelation->isNegated()){
        ...
        *out << ind++ << "for(auto undefKey = undefAggrVars["<<aggregateToStructure[aggregateRelation->getJoinTupleName()+sharedVariablesMap[aggregateIdentifier]+aggregateRelation->getAggrVarAsString()]<<"][{"<<sharedVariableTuple<<"}]."<<reverse_it<<"begin();undefKey!=undefAggrVars["<<aggregateToStructure[aggregateRelation->getJoinTupleName()+sharedVariablesMap[aggregateIdentifier]+aggregateRelation->getAggrVarAsString()]<<"][{"<<sharedVariableTuple<<"}]."<<reverse_it<<"end();undefKey++){\n";
        ...
        if(aggregateRelation->getAggregate().isSum()){
            std::string plusOne = aggregateRelation->isPlusOne() ? "+1":"";
            *out << ind++ << "if(undefPlusTrue-undefKey->at(0)"<<aggregateRelation->getCompareTypeAsString()<<aggregateRelation->getGuard().getStringRep()<<plusOne<<")\n";
            *out << ind-- << "break;\n";
            *out << ind++ << "else{\n";
            pars++;
        ...
        *out << ind++ << "if(undefinedTuples->size()==1){\n"<<std::endl;
            ...
            *out << ind++ << "if(tuple"<<litIndex<<"!=NULL){\n";
                ...
                *out << ind++ << "if(it"<<litIndex<<" != tupleToVar.end()) {\n";
                    *out << ind << "propagated=true;\n";
                    ...
                    //     //  *out << ind << "for(int v : local_reason) {if (v < 0){ std::cout<<\"-\"; v*=-1;}atomsTable[v].print();}\n";
        ...
        //change iteration 
        //  iterate UndefAggrVars and < case break
        // if(sharedVariablesMap[aggregateIdentifier]!=""){
        //     std::string sharedVarIndex = "";
        //     for(unsigned v : sharedVariablesIndexesMap[aggregateIdentifier]){
        //         sharedVarIndex+=std::to_string(v)+"_";
        //     }
        //     *out << ind << "const std::vector<const Tuple*>* undefinedTuples = &u_"<<joinTupleName<<sharedVarIndex<<".getValues({"<<sharedVariablesMap[aggregateIdentifier]<<"});\n";
        // }else{
        ...
        *out << ind++ << "for(auto undefKey = undefAggrVars["<<aggregateToStructure[aggregateRelation->getJoinTupleName()+sharedVariablesMap[aggregateIdentifier]+aggregateRelation->getAggrVarAsString()]<<"][{"<<sharedVariableTuple<<"}]."<<reverse_it<<"begin();undefKey!=undefAggrVars["<<aggregateToStructure[aggregateRelation->getJoinTupleName()+sharedVariablesMap[aggregateIdentifier]+aggregateRelation->getAggrVarAsString()]<<"][{"<<sharedVariableTuple<<"}]."<<reverse_it<<"end();undefKey++){\n";
        ...
        if(aggregateRelation->getAggregate().isSum()){
            std::string plusOne = aggregateRelation->isPlusOne() ? "+1":"";
            *out << ind++ << "if(actualSum["<<aggregateToStructure[aggregateRelation->getJoinTupleName()+sharedVariablesMap[aggregateIdentifier]+aggregateRelation->getAggrVarAsString()]<<"][{"<<sharedVariableTuple<<"}]+undefKey->at(0) < "<<aggregateRelation->getGuard().getStringRep()<<plusOne<<")\n";
            *out << ind-- << "break;\n";
            *out << ind++ << "else{\n";
            pars++;
        ...
        *out << ind++ << "for(int iUndef=0;iUndef<undefinedTuples->size();iUndef++){\n";
        ...
        // *out << ind << "std::vector<int> undefKey({";
        // bool atLeastOne=false;
        // for(unsigned v:aggregateVariablesIndex[aggregateIdentifier]){
        //     if(atLeastOne)
        //         *out << ",";
        //     *out << "undefinedTuples->at(iUndef)->at("<<v<<")";
        //     atLeastOne=true;
        // }
        // *out << "});\n";
        
        // *out << ind << "undefinedTuples->at(iUndef)->print();std::cout<<std::endl;\n";
        // *out << ind++ << "if(trueAggrVars["<<aggregateToStructure[aggregateRelation->getJoinTupleName()+sharedVariablesMap[aggregateIdentifier]+aggregateRelation->getAggrVarAsString()]<<"][{"<<sharedVariablesMap[aggregateIdentifier]<<"}].find(undefKey) == trueAggrVars["<<aggregateToStructure[aggregateRelation->getJoinTupleName()+sharedVariablesMap[aggregateIdentifier]+aggregateRelation->getAggrVarAsString()]<<"][{"<<sharedVariablesMap[aggregateIdentifier]<<"}].end()){\n";
        // pars++;
        
        // if(aggregateRelation->getAggregate().isSum()){
        //     std::string plusOne=aggregateRelation->isPlusOne() ? "+1":"";
        //     *out << ind++ << "if(actualSum["<<aggregateToStructure[aggregateRelation->getJoinTupleName()+sharedVariablesMap[aggregateIdentifier]+aggregateRelation->getAggrVarAsString()]<<"][{"<<sharedVariablesMap[aggregateIdentifier]<<"}]+undefKey[0] >= "<<aggregateRelation->getGuard().getStringRep()<<plusOne<<"){\n";
        //     pars++;
        // }
        int startingIndex=0;
        int litIndex=0;
    ...
    if(!aggr->getAggregate().isSum() && bound){
        if(aggr->isNegated()){
            ...
            *out << ind++ << "if(undefPlusTrue == "<<aggr->getGuard().getStringRep()<<plusOne<<"){\n";
            ...
            std::string plusOne = aggr->isPlusOne() ? "": "-1";
            *out << ind++ << "if(trueAggrVars["<<aggregateToStructure[aggr->getJoinTupleName()+sharedVariablesMap[aggrIdentifier]+aggr->getAggrVarAsString()]<<"][{"<<sharedVariablesMap[aggrIdentifier]<<"}].size() == "<<aggr->getGuard().getStringRep()<<plusOne<<"){\n";
        }
    }else {
        *out << ind++ <<"{\n";
    }
...
void CompilationManager::printAggregateTrueIf(std::string aggrIdentifier,const aspc::ArithmeticRelationWithAggregate* aggr,std::string joinTupleName,std::string op,bool isBound){
    ...
    if(!aggr->isNegated()){
        if(isBound)
            if(!aggr->getAggregate().isSum())
                *out << ind++ << "if(trueAggrVars["<<aggregateToStructure[aggr->getJoinTupleName()+sharedVariablesMap[aggrIdentifier]+aggr->getAggrVarAsString()]<<"][{"<<sharedVars<<"}].size()"<<aggr->getCompareTypeAsString()<<aggr->getGuard().getStringRep()<<plusOne<<"){\n";
            else
                *out << ind++ << "if(actualSum["<<aggregateToStructure[aggr->getJoinTupleName()+sharedVariablesMap[aggrIdentifier]+aggr->getAggrVarAsString()]<<"][{"<<sharedVars<<"}]"<<aggr->getCompareTypeAsString()<<aggr->getGuard().getStringRep()<<plusOne<<"){\n";
        else {
            if(!aggr->getAggregate().isSum())
                *out << ind << "int count"<<ruleId<<"_"<<aggr->getFormulaIndex()<<" = trueAggrVars["<<aggregateToStructure[aggr->getJoinTupleName()+sharedVariablesMap[aggrIdentifier]+aggr->getAggrVarAsString()]<<"][{"<<sharedVars<<"}].size();\n";
            else
                *out << ind << "int count"<<ruleId<<"_"<<aggr->getFormulaIndex()<<" = actualSum["<<aggregateToStructure[aggr->getJoinTupleName()+sharedVariablesMap[aggrIdentifier]+aggr->getAggrVarAsString()]<<"][{"<<sharedVars<<"}];\n";
            *out << ind++ << "{\n";
        }

    }else{
        if(aggr->getAggregate().isSum()){
            *out << ind << "int undefPlusTrue = actualSum["<<aggregateToStructure[aggr->getJoinTupleName()+sharedVariablesMap[aggrIdentifier]+aggr->getAggrVarAsString()]<<"][{"<<sharedVars<<"}]+possibleSum["<<aggregateToStructure[aggr->getJoinTupleName()+sharedVariablesMap[aggrIdentifier]+aggr->getAggrVarAsString()]<<"][{"<<sharedVars<<"}];\n";
        }else{
            *out << ind << "int undefPlusTrue = trueAggrVars["<<aggregateToStructure[aggr->getJoinTupleName()+sharedVariablesMap[aggrIdentifier]+aggr->getAggrVarAsString()]<<"][{"<<sharedVars<<"}].size()+undefAggrVars["<<aggregateToStructure[aggr->getJoinTupleName()+sharedVariablesMap[aggrIdentifier]+aggr->getAggrVarAsString()]<<"][{"<<sharedVars<<"}].size();\n";
        }
        if(isBound){
...
//            *out << ind << "std::cout<<\"Local reason size with aggr: \"<<local_reason.size();\n";
            ...
            *out << ind++ << "if(it!=tupleToVar.end()){\n";
                // *out << ind << "std::cout<<\"Adding starter\"<<std::endl;\n";
                *out << ind << "local_reason.push_back(it->second * (starter.isNegated() ? -1:1));\n";
            *out << --ind <<"}\n";
            for(int j=1;j<joinOrder.size()-aggregates.size();j++){
                if(body[joinOrder[j]]->isLiteral()){
                    ...
                    *out << ind++ << "if(tuple"<<j<<"!=tupleU){\n";
                        ...
                        *out << ind++ << "if(it!=tupleToVar.end()){\n";
                            if(l->isNegated())
                                *out << ind << "local_reason.push_back(it->second * -1);\n";
                            else
                                *out << ind << "local_reason.push_back(it->second);\n";
                            // *out << ind << "tuple"<<j<<"->print();std::cout<<\"Added to reason "<<j<<"\"<<std::endl;\n";
                        *out << --ind <<"}\n";
                    *out << --ind <<"}\n";
                }
                    ...
                    for(const aspc::ArithmeticRelationWithAggregate* other : aggregates){
                        ...
                        if(otherAggrIndex==propAggrIndex && other->isNegated()){
                            if(other->getAggregate().isSum()){
                                *out << ind << "int undefPlusTrue = actualSum["<<aggregateToStructure[other->getJoinTupleName()+sharedVariablesMap[identifier]+other->getAggrVarAsString()]<<"][{"<<localSharedVars<<"}]+possibleSum["<<aggregateToStructure[other->getJoinTupleName()+sharedVariablesMap[identifier]+other->getAggrVarAsString()]<<"][{"<<localSharedVars<<"}];\n";
                            }else{
                                *out << ind << "int undefPlusTrue = trueAggrVars["<<aggregateToStructure[other->getJoinTupleName()+sharedVariablesMap[identifier]+other->getAggrVarAsString()]<<"][{"<<localSharedVars<<"}].size()+undefAggrVars["<<aggregateToStructure[other->getJoinTupleName()+sharedVariablesMap[identifier]+other->getAggrVarAsString()]<<"][{"<<localSharedVars<<"}].size();\n";
                            }
            ...
            *out << ind << "tupleU=NULL;\n";
            ...
            if(!boundStarter){
                ...
                printAggregateTrueIf(aggrIdentifier,starter,joinTupleName,op,true);
                    ...
                    // else
                    //     *out << ind << "std::cout << actualSum["<<starter->getJoinTupleName()<<sharedVariablesMap[aggrIdentifier]<<"\"][{"<<sharedVariablesMap[aggrIdentifier]<<"}]<<\" Starting true <<std::endl;\n";
                    if(reason){
                        *out << ind << "std::vector<int> local_reason;\n";
                        for(const aspc::ArithmeticRelationWithAggregate& aggr : r.getArithmeticRelationsWithAggregate()){
                            std::string identifier = std::to_string(r.getRuleId())+":"+std::to_string(aggr.getFormulaIndex());
...
//            *out << ind << "std::cout<<\"Local reason size with aggr: \"<<local_reason.size();\n";
        ...
        // *out << ind << "std::cout<<\"Out Starting from aggr\"<<std::endl;\n";
}
