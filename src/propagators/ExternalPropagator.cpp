    ...
    Literal currentLiteral )
{
    ...
    else
    {
        ...
        // std::cout<<"getReasonForLiteral"<<std::endl;
    }
...
// {
//     std::cout<<"clearClausesToDelete "<<clausesToDelete.size()<<std::endl;
//     unsigned int totalLenght = 0;
//     for( unsigned int i = 0; i < clausesToDelete.size(); i++ ){
//         delete clausesToDelete[ i ];
//         totalLenght+=clausesToDelete[ i ]->size();
//     }
//     std::cout<<"AVG lenght for clauses "<<totalLenght<<std::endl;
//     clausesToDelete.clear();
// }
void ExternalPropagator::clearClausesToDelete(unsigned int level)
{
    unsigned int totalLenght = 0;
    for( unsigned int i = level+1; i < clausesToDelete.size(); i++ ) {
        for( unsigned int j = 0; j < clausesToDelete[i].size(); j++ ){
            totalLenght+=clausesToDelete[ i ][ j ]->size();
